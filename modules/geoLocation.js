...
const NIGERIAN_LANGS = ["ha", "yo", "ig"];

// Checking language
export const getSupportedLanguage = (countryCode) => {
  if (countryCode === "NG") {
    const browserLang = (navigator.language || "").split("-")[0].toLowerCase();
    return NIGERIAN_LANGS.includes(browserLang) ? browserLang : "ha";
  }
...
export const getInitialLanguage = () => {
  ...
  // SupportedLanguages в data.js этого ленда расходится со словарями (там
  // dk/ee/lg и нет da/et/el/nl/…), поэтому проверяем по самому словарю модалки.
