...
export function setNewBonusBasedOnParams() {
  if (landType) {
    document.querySelector(".form-type-buttons").style.gridTemplateColumns =
      "1fr";
    if (!onlyPhone) {
      const hiddenTab = formTab === "phone" ? "email" : "phone";
      document
        .querySelector(`.socials-form-type-btn[data-tab='${hiddenTab}']`)
...
// одна из вкладок скрыта во всех ветках выше — оставшейся клик не нужен
syncFormTabsInteractivity();
  ...
  if (modal) {
    ...
    if (formStep1) {
      ...
      const recalcPhoneBtn = () => {
        if (formTab === "phone")
          formStepBtnNext.disabled = !isPhoneFieldValid();
      };
    ...
    if (formStep2) {
      ...
      // Единая точка расчёта состояния кнопки шага 2. Раньше условия жили в
      // разных обработчиках: ввод и blur пароля смотрели ТОЛЬКО его длину и
      // включали кнопку при снятом согласии, а change чекбокса — отдельно.
      // Форма уходила без согласия: снять галочку и дописать символ в пароль.
      // Правило одно для обеих вкладок: согласие + пароль от 6 символов (на
      // вкладке phone пароль тоже уходит в /api/register).
      const isStep2Valid = () =>
