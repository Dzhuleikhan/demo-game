      ...
      if (formStepCount === 2) {
        if (!submitBtn.disabled) {
          if (formTab === "email") {
            ...
            window.location.href = `https://${newDomain}/api/register?env=prod&game=platipus:hotfruits&mode=demo&type=${formTab}&currency=${formData.currency}&email=${encodeURIComponent(formData.email)}&password=${encodeURIComponent(formData.password)}${formData.bonus === "" ? "" : "&bonus=" + formData.bonus}&lang=${lang}${cid ? "&cid=" + cid : ""}`;
            console.log(
              `https://${newDomain}/api/register?env=prod&game=platipus:hotfruits&mode=demo&type=${formTab}&currency=${formData.currency}&email=${encodeURIComponent(formData.email)}&password=${encodeURIComponent(formData.password)}${formData.bonus === "" ? "" : "&bonus=" + formData.bonus}&lang=${lang}${cid ? "&cid=" + cid : ""}`,
            );
          } else if (formTab === "phone") {
            disableFormWhileSubmitting();
            window.location.href = `https://${newDomain}/api/register?env=prod&game=platipus:hotfruits&mode=demo&type=${formTab}&currency=${formData.currency}&phone=${formData.phone}&password=${encodeURIComponent(formData.password)}${formData.bonus === "" ? "" : "&bonus=" + formData.bonus}&lang=${lang}${cid ? "&cid=" + cid : ""}`;
            console.log(
              `https://${newDomain}/api/register?env=prod&game=platipus:hotfruits&mode=demo&type=${formTab}&currency=${formData.currency}&phone=${formData.phone}&password=${encodeURIComponent(formData.password)}${formData.bonus === "" ? "" : "&bonus=" + formData.bonus}&lang=${lang}${cid ? "&cid=" + cid : ""}`,
            );
          }
        }
      }
...
formSocialLinks.forEach((link) => {
  if (link) {
    link.addEventListener("click", (e) => {
      ...
      window.location.href = `https://${newDomain}/api/register?env=prod&game=platipus:hotfruits&mode=demo&type=${type}&currency=${currency}${bonus === "" ? "" : "&bonus=" + bonus}&lang=${lang}${cid ? "&cid=" + cid : ""}`;
      console.log(
        `https://${newDomain}/api/register?env=prod&game=platipus:hotfruits&mode=demo&type=${type}&currency=${currency}${bonus === "" ? "" : "&bonus=" + bonus}&lang=${lang}${cid ? "&cid=" + cid : ""}`,
      );
    });
  }
});
